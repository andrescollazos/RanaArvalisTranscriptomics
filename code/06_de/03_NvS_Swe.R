...
ggplot(res_df, aes(x = baseMean, y = log2FoldChange, color = diffexp_sig)) +
  ...
  labs(
    title = "MA plot: Region effect in Swedish populations",
...
EnhancedVolcano(
  ...
  title = 'Volcano plot: region effect in Swedish populations',
...
plot_pca(
  pca12,
  "PC1", "PC2",
  "PCA of Swedish samples: family, temperature, and region structure"
)
...
for (pop in unique(annotation_col$region)) {
  
  idx <- annotation_col$region == pop
  
  mat_pop <- mat_scaled[, idx]
  
  # Remove genes with zero variance within this region
  ...
  pheatmap(
    ...
    show_rownames = FALSE,
    fontsize_col = 8,
    main = paste("Region response in population", pop)
  )
}
